...
class EncoderTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # stage 1
        x1, H1, W1 = self.patch_embed1(x) # [B, H1*W1, embed_dims[0]]
        x2, H2, W2 = self.mini_patch_embed1(x1.permute(0,2,1).reshape(B,embed_dims[0],H1,W1)) # [B, embed_dims[1], H2, W2]

        for i, blk in enumerate(self.block1):
            x1 = blk(x1, H1, W1)
        x1 = self.norm1(x1) # [B, embed_dims[0], H1, W1]
        x1 = x1.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous() # [B, H1*W1, embed_dims[0]]
        
        for i, blk in enumerate(self.patch_block1):
            x2 = blk(x2, H2, W2)
        x2 = self.pnorm1(x2) # [B, embed_dims[1], H2, W2]
        x2 = x2.reshape(B, H2, W2, -1).permute(0, 3, 1, 2).contiguous() # [B, H2*W2, embed_dims[1]]

        outs.append(x1) 

        # stage 2
        x1, H1, W1 = self.patch_embed2(x1) # [B, embed_dims[1], H1, W1]
        x1 = x1.permute(0,2,1).reshape(B,embed_dims[1],H1,W1)+x2 # [B, embed_dims[1], H1, W1]
        x2, H2, W2 = self.mini_patch_embed2(x1) # [B, embed_dims[2], H2, W2]

        x1 = x1.view(x1.shape[0],x1.shape[1],-1).permute(0,2,1) # [B, H1*W1, embed_dims[1]]

        for i, blk in enumerate(self.block2):
            x1 = blk(x1, H1, W1)
        x1 = self.norm2(x1) # [B, embed_dims[1], H1, W1]
        x1 = x1.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous() # [B, H1*W1, embed_dims[1]]
        outs.append(x1)

        for i, blk in enumerate(self.patch_block2):
            x2 = blk(x2, H2, W2)
        x2 = self.pnorm2(x2) # [B, embed_dims[2], H2, W2]
        x2 = x2.reshape(B, H2, W2, -1).permute(0, 3, 1, 2).contiguous() # [B, H2*W2, embed_dims[2]]

        
        # stage 3
        x1, H1, W1 = self.patch_embed3(x1) # [B, embed_dims[2], H1, W1]
        x1 = x1.permute(0,2,1).reshape(B,embed_dims[2],H1,W1)+x2 # [B, embed_dims[2], H1, W1]
        x2, H2, W2 = self.mini_patch_embed3(x1) # [B, embed_dims[3], H2, W2]

        x1 = x1.view(x1.shape[0],x1.shape[1],-1).permute(0,2,1)

        for i, blk in enumerate(self.block3):
            x1 = blk(x1, H1, W1)
        x1 = self.norm3(x1) # [B, embed_dims[2], H1, W1]
        x1 = x1.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous() # [B, H1*W1, embed_dims[2]]
        outs.append(x1)
        
        for i, blk in enumerate(self.patch_block3):
            x2 = blk(x2, H2, W2)
        x2 = self.pnorm3(x2) # [B, embed_dims[3], H2, W2]
        x2 = x2.reshape(B, H2, W2, -1).permute(0, 3, 1, 2).contiguous() # [B, H2*W2, embed_dims[3]]

        # stage 4
        x1, H1, W1 = self.patch_embed4(x1) # [B, embed_dims[3], H1, W1]
        x1 = x1.permute(0,2,1).reshape(B,embed_dims[3],H1,W1)+x2 # [B, embed_dims[3], H1, W1]

        x1 = x1.view(x1.shape[0],x1.shape[1],-1).permute(0,2,1) # [B, H1*W1, embed_dims[3]]

        for i, blk in enumerate(self.block4):
            x1 = blk(x1, H1, W1)
        x1 = self.norm4(x1) # [B, embed_dims[3], H1, W1]
        x1 = x1.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous() # [B, H1*W1, embed_dims[3]]
        outs.append(x1)

        return outs # [B, H1*W1, embed_dims[0]], [B, H1*W1, embed_dims[1]], [B, H1*W1, embed_dims[2]], [B, H1*W1, embed_dims[3]]
...
class OverlapPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        return x, H, W # [B, H*W, embed_dim], H, W
...
class Tenc(EncoderTransformer):
    def __init__(self, **kwargs):
        super(Tenc, self).__init__(
            patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[1, 2, 4, 4], mlp_ratios=[2, 2, 2, 2],
